...
\fancyfoot[C]{\thepage}
	...
	\end{cases}
	\]
	Найти $A$, вероятность попадания СВ $X$ в интервал $(-1; 1{,}5)$
  ...
  \end{center}
    ...
    \draw[dashed] (-1,0) -- (-1,1);
		...
		% Стрелки-указатели уровней, как на рисунке
		\draw[->] (-3,0) -- (-2,0);
		\draw[->] (-2,1) -- (-1,1);
		\draw[->] (-1,2) -- (0,2);
	\end{tikzpicture}
  ...
  \end{center}
	...
	\noindent$3.$ Случайная величина $X \sim \mathbf{N} (1; 5)$.
	Найти $b$, если известно, что
	\[
		P\{-3 < X < b\} = 0{,}604
	\]
  ...
  \end{center}
	...
	\noindent$4.$ Известно, что на предприятиях электронной промышленности
	$15\%$ работников имеют степень бакалавра.
	Для некоторого обследования наугад выбрано $5000$ работников.
	Найти вероятность того, что в отобранной группе окажется более
	$770$ работников-бакалавров.
