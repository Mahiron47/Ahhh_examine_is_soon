...
public class TextRenderer {
	...
	class Drawer implements CharacterVisitor {
		...
		public Drawer(
			...
			this.red = (color >> 16 & 0xFF) / 255.0F * this.brightnessMultiplier;
			this.green = (color >> 8 & 0xFF) / 255.0F * this.brightnessMultiplier;
			this.blue = (color & 0xFF) / 255.0F * this.brightnessMultiplier;
		...
		public boolean accept(int i, Style style, int j) {
			...
			if (textColor != null) {
				...
				g = (k >> 16 & 0xFF) / 255.0F * this.brightnessMultiplier;
				h = (k >> 8 & 0xFF) / 255.0F * this.brightnessMultiplier;
				l = (k & 0xFF) / 255.0F * this.brightnessMultiplier;
			} else {
				g = this.red;
				h = this.green;
				l = this.blue;
		...
		public float drawLayer(int underlineColor, float x) {
			if (underlineColor != 0) {
				float f = (underlineColor >> 24 & 0xFF) / 255.0F;
				float g = (underlineColor >> 16 & 0xFF) / 255.0F;
				float h = (underlineColor >> 8 & 0xFF) / 255.0F;
				float i = (underlineColor & 0xFF) / 255.0F;
