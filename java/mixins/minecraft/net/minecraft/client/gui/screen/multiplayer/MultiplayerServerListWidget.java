	...
	public class ServerEntry extends MultiplayerServerListWidget.Entry {
		...
		public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
			...
			if (bl) {
				...
				k = 1;
				l = (int)(Util.getMeasuringTimeMs() / 100L + index * 2 & 7L);
				if (l > 4) {
					l = 8 - l;
				}
			...
			context.drawTexture(MultiplayerServerListWidget.ICONS_TEXTURE, x + entryWidth - 15, y, k * 10, 176 + l * 8, 10, 8, 256, 256);
