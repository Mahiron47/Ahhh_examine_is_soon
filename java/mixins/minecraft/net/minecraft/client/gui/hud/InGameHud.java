...
public class InGameHud {
	...
	public void render(DrawContext context, float tickDelta) {
		...
		if (!this.client.options.hudHidden) {
			if (this.overlayMessage != null && this.overlayRemaining > 0) {
				...
				float h = this.overlayRemaining - tickDelta;
			...
			if (this.title != null && this.titleRemainTicks > 0) {
				...
				float hx = this.titleRemainTicks - tickDelta;
				int lx = 255;
				if (this.titleRemainTicks > this.titleFadeOutTicks + this.titleStayTicks) {
					float o = this.titleFadeInTicks + this.titleStayTicks + this.titleFadeOutTicks - hx;
					lx = (int)(o * 255.0F / this.titleFadeInTicks);
				}

				if (this.titleRemainTicks <= this.titleFadeOutTicks) {
					lx = (int)(hx * 255.0F / this.titleFadeOutTicks);
				}
			...
			RenderSystem.enableBlend();
			int n = MathHelper.floor(this.client.mouse.getX() * window.getScaledWidth() / window.getWidth());
			int p = MathHelper.floor(this.client.mouse.getY() * window.getScaledHeight() / window.getHeight());
	...
	protected void renderStatusEffectOverlay(DrawContext context) {
		...
		if (!collection.isEmpty()
			&& !(this.client.currentScreen instanceof AbstractInventoryScreen abstractInventoryScreen && abstractInventoryScreen.hideStatusEffectHud())) {
			RenderSystem.enableBlend();
			int i = 0;
			int j = 0;
	...
	private void renderStatusBars(DrawContext context) {
		...
		if (playerEntity != null) {
			...
			if (playerEntity.isSubmergedIn(FluidTags.WATER) || zx < y) {
				...
				t -= aax * 10;
				int abx = MathHelper.ceil((zx - 2) * 10.0 / y);
				int acx = MathHelper.ceil(zx * 10.0 / y) - abx;
	...
	private void renderHealthBar(
		...
		int j = MathHelper.ceil(maxHealth / 2.0);
		int k = MathHelper.ceil(absorption / 2.0);
		int l = j * 2;
	...
	private void renderSpyglassOverlay(DrawContext context, float scale) {
		...
		float h = Math.min(this.scaledWidth / f, this.scaledHeight / f) * scale;
	...
	private void renderHotbarItem(DrawContext context, int x, int y, float f, PlayerEntity player, ItemStack stack, int seed) {
		if (!stack.isEmpty()) {
			float g = stack.getBobbingAnimationTime() - f;
			if (g > 0.0F) {
				float h = 1.0F + g / 5.0F;
				context.getMatrices().push();
