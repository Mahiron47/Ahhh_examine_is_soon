...
public class EditBoxWidget extends ScrollableWidget {
	...
	protected boolean overflows() {
		return this.editBox.getLineCount() > this.getMaxLinesWithoutOverflow();
	}
	...
	private void onCursorChange() {
		...
		if (this.editBox.getCursor() <= substring.beginIndex()) {
			...
			if (this.editBox.getCursor() > substring2.endIndex()) {
				d = this.editBox.getCurrentLineIndex() * 9 - this.height + 9 + this.getPaddingDoubled();
			}
		}
	...
	private double getMaxLinesWithoutOverflow() {
		return (this.height - this.getPaddingDoubled()) / 9.0;
	}

	private void moveCursor(double mouseX, double mouseY) {
		double d = mouseX - this.getX() - this.getPadding();
		double e = mouseY - this.getY() - this.getPadding() + this.getScrollY();
