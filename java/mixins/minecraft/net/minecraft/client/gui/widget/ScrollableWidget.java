...
public abstract class ScrollableWidget extends ClickableWidget implements Drawable, Element {
	...
	public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
		...
		if (bl || bl2) {
			...
			this.setScrollY(this.scrollY + (bl ? -1 : 1) * this.getDeltaYPerScroll());
			if (d != this.scrollY) {
				return true;
			}
		}
	...
	private int getScrollbarThumbHeight() {
		return MathHelper.clamp((int)((float)(this.height * this.height) / this.getContentsHeightWithPadding()), 32, this.height);
	}
	...
	protected boolean isVisible(int top, int bottom) {
		return bottom - this.scrollY >= this.getY() && top - this.scrollY <= this.getY() + this.height;
	}
