...
public abstract class EntryListWidget<E extends EntryListWidget.Entry<E>> extends AbstractParentElement implements Drawable, Selectable {
	...
	protected final E getEntryAtPosition(double x, double y) {
		...
		int k = j - i;
		int l = j + i;
		int m = MathHelper.floor(y - this.top) - this.headerHeight + (int)this.getScrollAmount() - 4;
		int n = m / this.itemHeight;
		return (E)(x < this.getScrollbarPositionX() && x >= k && x <= l && n >= 0 && m >= 0 && n < this.getEntryCount() ? this.children().get(n) : null);
	}
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		if (m > 0) {
			int n = (int)((float)((this.bottom - this.top) * (this.bottom - this.top)) / this.getMaxPosition());
	...
	protected void updateScrollingState(double mouseX, double mouseY, int button) {
		this.scrolling = button == 0 && mouseX >= this.getScrollbarPositionX() && mouseX < this.getScrollbarPositionX() + 6;
	}
	...
	public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
		if (super.mouseDragged(mouseX, mouseY, button, deltaX, deltaY)) {
			...
			if (mouseY < this.top) {
				...
				int j = MathHelper.clamp((int)((float)(i * i) / this.getMaxPosition()), 32, i - 8);
				double e = Math.max(1.0, d / (i - j));
