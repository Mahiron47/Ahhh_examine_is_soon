...
public class DrawContext {
	...
	private void fillGradient(VertexConsumer vertexConsumer, int startX, int startY, int endX, int endY, int z, int colorStart, int colorEnd) {
		float f = ColorHelper.Argb.getAlpha(colorStart) / 255.0F;
		float g = ColorHelper.Argb.getRed(colorStart) / 255.0F;
		float h = ColorHelper.Argb.getGreen(colorStart) / 255.0F;
		float i = ColorHelper.Argb.getBlue(colorStart) / 255.0F;
		float j = ColorHelper.Argb.getAlpha(colorEnd) / 255.0F;
		float k = ColorHelper.Argb.getRed(colorEnd) / 255.0F;
		float l = ColorHelper.Argb.getGreen(colorEnd) / 255.0F;
		float m = ColorHelper.Argb.getBlue(colorEnd) / 255.0F;
		Matrix4f matrix4f = this.matrices.peek().getPositionMatrix();
		vertexConsumer.vertex(matrix4f, startX, startY, z).color(g, h, i, f).next();
		vertexConsumer.vertex(matrix4f, startX, endY, z).color(k, l, m, j).next();
		vertexConsumer.vertex(matrix4f, endX, endY, z).color(k, l, m, j).next();
		vertexConsumer.vertex(matrix4f, endX, startY, z).color(g, h, i, f).next();
	}
	...
	void drawTexture(
		...
		this.drawTexturedQuad(
			texture, x1, x2, y1, y2, z, (u + 0.0F) / textureWidth, (u + regionWidth) / textureWidth, (v + 0.0F) / textureHeight, (v + regionHeight) / textureHeight
		);
	}
