...
public final class NativeImage implements AutoCloseable {
	...
	public void setLuminance(int x, int y, byte luminance) {
		...
		if (!this.format.hasLuminance()) {
			...
			this.checkAllocated();
			long l = (x + (long)y * this.width) * this.format.getChannelCount() + this.format.getLuminanceOffset() / 8;
	...
	public void blend(int x, int y, int color) {
		if (this.format != NativeImage.Format.RGBA) {
			...
			float f = ColorHelper.Abgr.getAlpha(color) / 255.0F;
			float g = ColorHelper.Abgr.getBlue(color) / 255.0F;
			float h = ColorHelper.Abgr.getGreen(color) / 255.0F;
			float j = ColorHelper.Abgr.getRed(color) / 255.0F;
			float k = ColorHelper.Abgr.getAlpha(i) / 255.0F;
			float l = ColorHelper.Abgr.getBlue(i) / 255.0F;
			float m = ColorHelper.Abgr.getGreen(i) / 255.0F;
			float n = ColorHelper.Abgr.getRed(i) / 255.0F;
			float p = 1.0F - f;
			float q = f * f + k * p;
			float r = g * f + l * p;
			float s = h * f + m * p;
	...
	public void makeGlyphBitmapSubpixel(
		...
		if (startX < 0 || startX + width > this.getWidth() || startY < 0 || startY + height > this.getHeight()) {
			...
			STBTruetype.nstbtt_MakeGlyphBitmapSubpixel(
				fontInfo.address(), this.pointer + startX + startY * this.getWidth(), width, height, this.getWidth(), scaleX, scaleY, shiftX, shiftY, glyphIndex
			);
		}
	}
