...
public class MinecraftClient extends ReentrantThreadExecutor<Runnable> implements WindowEventHandler {
	...
	private void drawProfilerResults(DrawContext context, ProfileResult profileResult) {
		...
		bufferBuilder.vertex(j - 176.0F, k - 96.0F - 16.0F, 0.0).color(200, 0, 0, 0).next();
		bufferBuilder.vertex(j - 176.0F, k + 320, 0.0).color(200, 0, 0, 0).next();
		bufferBuilder.vertex(j + 176.0F, k + 320, 0.0).color(200, 0, 0, 0).next();
		bufferBuilder.vertex(j + 176.0F, k - 96.0F - 16.0F, 0.0).color(200, 0, 0, 0).next();
		tessellator.draw();
		...
		for (ProfilerTiming profilerTiming2 : list) {
			...
			for (int q = l; q >= 0; q--) {
				float f = (float)((d + profilerTiming2.parentSectionUsagePercentage * q / l) * (float) (Math.PI * 2) / 100.0);
				float g = MathHelper.sin(f) * 160.0F;
				float h = MathHelper.cos(f) * 160.0F * 0.5F;
				bufferBuilder.vertex(j + g, k - h, 0.0).color(n, o, p, 255).next();
			}
			...
			for (int q = l; q >= 0; q--) {
				...
				float h = MathHelper.cos(f) * 160.0F * 0.5F;
				if (!(h > 0.0F)) {
					bufferBuilder.vertex(j + g, k - h, 0.0).color(n >> 1, o >> 1, p >> 1, 255).next();
					bufferBuilder.vertex(j + g, k - h + 10.0F, 0.0).color(n >> 1, o >> 1, p >> 1, 255).next();
				}
			}
