...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
	...
	public void init() {
		...
		if (this.getWorld() != null) {
			for (double d = this.getY(); d > this.getWorld().getBottomY() && d < this.getWorld().getTopY(); d++) {
	...
	public void tickMovement() {
		...
		if (this.getAbilities().flying && this.isCamera()) {
			...
			if (i != 0) {
				this.setVelocity(this.getVelocity().add(0.0, i * this.getAbilities().getFlySpeed() * 3.0F, 0.0));
			}
		}
