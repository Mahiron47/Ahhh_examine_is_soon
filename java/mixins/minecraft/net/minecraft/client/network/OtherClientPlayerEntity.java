...
public class OtherClientPlayerEntity extends AbstractClientPlayerEntity {
	...
	public void tickMovement() {
		if (this.bodyTrackingIncrements > 0) {
			double d = this.getX() + (this.serverX - this.getX()) / this.bodyTrackingIncrements;
			double e = this.getY() + (this.serverY - this.getY()) / this.bodyTrackingIncrements;
			double f = this.getZ() + (this.serverZ - this.getZ()) / this.bodyTrackingIncrements;
			this.setYaw(this.getYaw() + (float)MathHelper.wrapDegrees(this.serverYaw - this.getYaw()) / this.bodyTrackingIncrements);
			this.setPitch(this.getPitch() + (float)(this.serverPitch - this.getPitch()) / this.bodyTrackingIncrements);
		...
		if (this.headTrackingIncrements > 0) {
			this.headYaw = this.headYaw + (float)(MathHelper.wrapDegrees(this.serverHeadYaw - this.headYaw) / this.headTrackingIncrements);
