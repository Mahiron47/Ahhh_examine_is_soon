...
public class HeldItemRenderer {
	...
	private void applyEatOrDrinkTransformation(MatrixStack matrices, float tickDelta, Arm arm, ItemStack stack) {
		float f = this.client.player.getItemUseTimeLeft() - tickDelta + 1.0F;
		...
		if (!player.isUsingSpyglass()) {
			...
			if (item.isEmpty()) {
				...
				if (player.isUsingItem() && player.getItemUseTimeLeft() > 0 && player.getActiveHand() == hand) {
					...
					float f = item.getMaxUseTime() - (this.client.player.getItemUseTimeLeft() - tickDelta + 1.0F);
					float g = f / CrossbowItem.getPullTime(item);
					if (g > 1.0F) {
						g = 1.0F;
					}
				...
				if (player.isUsingItem() && player.getItemUseTimeLeft() > 0 && player.getActiveHand() == hand) {
					...
					switch (item.getUseAction()) {
						...
						case BOW:
							...
							float mx = item.getMaxUseTime() - (this.client.player.getItemUseTimeLeft() - tickDelta + 1.0F);
							float fxx = mx / 20.0F;
						...
						case SPEAR:
							...
							matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(l * 35.3F));
							matrices.multiply(RotationAxis.POSITIVE_Z.rotationDegrees(l * -9.785F));
							float m = item.getMaxUseTime() - (this.client.player.getItemUseTimeLeft() - tickDelta + 1.0F);
							float fx = m / 10.0F;
							if (fx > 1.0F) {
								fx = 1.0F;
							}
