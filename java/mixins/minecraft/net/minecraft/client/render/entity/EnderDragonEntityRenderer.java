...
public class EnderDragonEntityRenderer extends EntityRenderer<EnderDragonEntity> {
	...
	public void render(EnderDragonEntity enderDragonEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
		...
		if (enderDragonEntity.ticksSinceDeath > 0) {
			float k = enderDragonEntity.ticksSinceDeath / 200.0F;
		...
		if (enderDragonEntity.ticksSinceDeath > 0) {
			float l = (enderDragonEntity.ticksSinceDeath + g) / 200.0F;
			...
			for (int n = 0; n < (l + l * l) / 2.0F * 60.0F; n++) {
	...
	public static void renderCrystalBeam(
		...
		float i = MathHelper.sqrt(dx * dx + dy * dy + dz * dz) / 32.0F - (age + tickDelta) * 0.01F;
		int j = 8;
		float k = 0.0F;
		float l = 0.75F;
		float m = 0.0F;
	...
	public static class DragonEntityModel extends EntityModel<EnderDragonEntity> {
		...
		public void render(MatrixStack matrices, VertexConsumer vertices, int light, int overlay, float red, float green, float blue, float alpha) {
			...
			float m = MathHelper.wrapDegrees((float)(this.dragon.getSegmentProperties(5, this.tickDelta)[0] + l / 2.0F));
			...
			for (int o = 0; o < 5; o++) {
				...
				this.neck.roll = -MathHelper.wrapDegrees((float)(es[0] - m)) * (float) (Math.PI / 180.0) * 1.5F;
				this.neck.pivotY = i;
				this.neck.pivotZ = j;
				this.neck.pivotX = h;
			...
			this.head.roll = -MathHelper.wrapDegrees((float)(fs[0] - m)) * (float) (Math.PI / 180.0);
			...
			for (int r = 0; r < 12; r++) {
				...
				p += MathHelper.sin(r * 0.45F + n) * 0.05F;
				...
				this.neck.roll = MathHelper.wrapDegrees((float)(fs[0] - m)) * (float) (Math.PI / 180.0) * 1.5F;
				this.neck.pivotY = i;
				this.neck.pivotZ = j;
				this.neck.pivotX = h;
				i += MathHelper.sin(this.neck.pitch) * 10.0F;
