	...
	public void render(BlockRenderView world, BlockPos pos, VertexConsumer vertexConsumer, BlockState blockState, FluidState fluidState) {
		...
		if (bl2 || bl3 || bl7 || bl6 || bl4 || bl5) {
			...
			if (bl2 && !isSideCovered(world, pos, Direction.UP, Math.min(Math.min(p, r), Math.min(q, o)), blockState3)) {
				...
				float an = k * g;
				float ao = k * h;
				this.vertex(vertexConsumer, d + 0.0, e + p, w + 0.0, ak, an, ao, z, aa, am);
				this.vertex(vertexConsumer, d + 0.0, e + r, w + 1.0, ak, an, ao, ab, ac, am);
				this.vertex(vertexConsumer, d + 1.0, e + q, w + 1.0, ak, an, ao, ad, ae, am);
				this.vertex(vertexConsumer, d + 1.0, e + o, w + 0.0, ak, an, ao, af, ag, am);
			...
			if (bl3) {
				...
				float aex = j * g;
				float agx = j * h;
				this.vertex(vertexConsumer, d, e + y, w + 1.0, acx, aex, agx, zx, afx, ap);
				this.vertex(vertexConsumer, d, e + y, w, acx, aex, agx, zx, adx, ap);
				this.vertex(vertexConsumer, d + 1.0, e + y, w, acx, aex, agx, abx, adx, ap);
				this.vertex(vertexConsumer, d + 1.0, e + y, w + 1.0, acx, aex, agx, abx, afx, ap);
			}
			...
			for (Direction direction : Direction.Type.HORIZONTAL) {
				...
				if (bl8 && !isSideCovered(world, pos, direction, Math.max(afx, aax), world.getBlockState(pos.offset(direction)))) {
					...
					float bc = k * ba * g;
					float bd = k * ba * h;
					this.vertex(vertexConsumer, ar, e + afx, at, bb, bc, bd, av, ax, aq);
					this.vertex(vertexConsumer, as, e + aax, au, bb, bc, bd, aw, ay, aq);
					this.vertex(vertexConsumer, as, e + y, au, bb, bc, bd, aw, az, aq);
					this.vertex(vertexConsumer, ar, e + y, at, bb, bc, bd, av, az, aq);
