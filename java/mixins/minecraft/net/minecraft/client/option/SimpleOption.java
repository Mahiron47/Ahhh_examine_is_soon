...
public final class SimpleOption<T> {
	...
	interface IntSliderCallbacks extends SimpleOption.SliderCallbacks<Integer> {
		...
		default double toSliderProgress(Integer integer) {
			return MathHelper.map((float)integer.intValue(), (float)this.minInclusive(), (float)this.maxInclusive(), 0.0F, 1.0F);
		}
	 ...
	 */
	@Environment(EnvType.CLIENT)
	public record ValidatingIntSliderCallbacks(int minInclusive, int maxInclusive) implements SimpleOption.IntSliderCallbacks {
		public Optional<Integer> validate(Integer integer) {
