...
public abstract class PlayerEntity extends LivingEntity {
	...
	private boolean method_30263() {
		return this.isOnGround()
			|| this.fallDistance < this.getStepHeight()
				&& !this.getWorld().isSpaceEmpty(this, this.getBoundingBox().offset(0.0, this.fallDistance - this.getStepHeight(), 0.0));
	}
		...
		if (target.isAttackable()) {
			if (!target.handleAttack(this)) {
				...
				if (f > 0.0F || g > 0.0F) {
					...
					if (bl && !bl3 && !bl2 && this.isOnGround() && d < this.getMovementSpeed()) {
					...
					if (bl6) {
						if (i > 0) {
							if (target instanceof LivingEntity) {
								((LivingEntity)target)
									.takeKnockback(i * 0.5F, MathHelper.sin(this.getYaw() * (float) (Math.PI / 180.0)), -MathHelper.cos(this.getYaw() * (float) (Math.PI / 180.0)));
							} else {
								target.addVelocity(
									-MathHelper.sin(this.getYaw() * (float) (Math.PI / 180.0)) * i * 0.5F, 0.1, MathHelper.cos(this.getYaw() * (float) (Math.PI / 180.0)) * i * 0.5F
								);
							}
	...
	public void disableShield(boolean sprinting) {
		float f = 0.25F + EnchantmentHelper.getEfficiency(this) * 0.05F;
		if (sprinting) {
			f += 0.75F;
		}
	...
	public static Optional<Vec3d> findRespawnPosition(ServerWorld world, BlockPos pos, float angle, boolean forced, boolean alive) {
		...
		if (block instanceof RespawnAnchorBlock && (forced || (Integer)blockState.get(RespawnAnchorBlock.CHARGES) > 0) && RespawnAnchorBlock.isNether(world)) {
			...
			if (!forced && !alive && optional.isPresent()) {
				world.setBlockState(pos, blockState.with(RespawnAnchorBlock.CHARGES, (Integer)blockState.get(RespawnAnchorBlock.CHARGES) - 1), Block.NOTIFY_ALL);
			}
			...
			return bl && bl2 ? Optional.of(new Vec3d(pos.getX() + 0.5, pos.getY() + 0.1, pos.getZ() + 0.5)) : Optional.empty();
		}
	}
