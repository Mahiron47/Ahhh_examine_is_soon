...
public class NetherPortal {
	...
	public void createPortal() {
		...
		BlockPos.iterate(this.lowerCorner, this.lowerCorner.offset(Direction.UP, this.height - 1).offset(this.negativeDir, this.width - 1))
			.forEach(blockPos -> this.world.setBlockState(blockPos, blockState, Block.FORCE_STATE | Block.NOTIFY_LISTENERS));
	}
	...
	public static Vec3d entityPosInPortal(BlockLocating.Rectangle portalRect, Direction.Axis portalAxis, Vec3d entityPos, EntityDimensions entityDimensions) {
		...
		double g;
		if (d > 0.0) {
			float f = blockPos.getComponentAlongAxis(portalAxis) + entityDimensions.width / 2.0F;
		...
		if (e > 0.0) {
			...
			h = MathHelper.clamp(MathHelper.getLerpProgress(entityPos.getComponentAlongAxis(axis) - blockPos.getComponentAlongAxis(axis), 0.0, e), 0.0, 1.0);
		} else {
			h = 0.0;
		}

		Direction.Axis axis = portalAxis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
		double i = entityPos.getComponentAlongAxis(axis) - (blockPos.getComponentAlongAxis(axis) + 0.5);
	...
	public static TeleportTarget getNetherTeleportTarget(
		...
		double f = entityDimensions.width / 2.0 + (d - entityDimensions.width) * offset.getX();
		double g = (e - entityDimensions.height) * offset.getY();
		double h = 0.5 + offset.getZ();
		boolean bl = axis == Direction.Axis.X;
		Vec3d vec3d2 = new Vec3d(blockPos.getX() + (bl ? f : h), blockPos.getY() + g, blockPos.getZ() + (bl ? h : f));
		Vec3d vec3d3 = findOpenPosition(vec3d2, destination, entity, entityDimensions);
		return new TeleportTarget(vec3d3, vec3d, yaw + i, pitch);
	}
