...
struct food {
    ...
    food() : name(nullptr), cost(0), quant(0) {}
    food(char* _name, double _cost, int _quant) : cost(_cost), quant(_quant) {
    ...
    food(const food& other) : cost(other.cost), quant(other.quant) {
        uint32_t size = strlen(other.name);
        name = new char[size + 1];
        for (uint32_t i = 0; i < size; i++) {
            name[i] = other.name[i];
        }
...
public:
    ...
    ~FoodExcel() {
        ...
        for (size_t i = 0; i < size; i++) {
            ...
            file << list[i].name << " " << list[i].cost << " " << list[i].quant << "\n";
        }
        file.close();
    ...
    void execute() {
        while (true) {
            ...
            std::cout << "Добавить новый элемент                                    …… 1" << std::endl
                      << "Распечатать базу товаров                                  …… 2" << std::endl
                      << "Поиск товара по названию                                  …… 3" << std::endl
                      << "Найти все продукты с количеством более заданного значения …… 4" << std::endl
...
private:
    ...
    food* find(double cost, size_t& _size) {
        food* _list = new food[size];
        _size = 0;
        for (size_t i = 0; i < size; i++) {
            if (list[i].cost > cost) {
    ...
    void print() {
        ...
        for (size_t i = 0; i < size; i++) {
            std::cout << [this](int number) -> char* {
                ...
                for (size_t i = 0; i < 20; i++) {
                    if ((int)list[number].name[i] < 0) {
                        if (i < strlen(list[number].name)) {
                            shift++;
                            result[i] = list[number].name[i];
                        } else {
                            if (shift % 2 == 0) {
                                result[i] = ' ';
                            }
                ...
                result[20 + shift] = ' ';
                long cost = list[number].cost * 100;
                for (int i = 29; i > 27; i--) {
                ...
                result[42 + shift] = ' ';
                long sum = list[number].cost * list[number].quant * 100;
                for (int i = 61; i > 59; i--) {
        ...
        std::cout << "--------------------------------------------------------------" << std::endl
                  << "Всего товаров на сумму" << [this]() -> char* {
            char* result = new char[41];
            double _sum = 0;
            for (size_t i = 0; i < size; i++) {
                _sum += list[i].cost * list[i].quant;
            }
            long sum = _sum * 100;
            for (int i = 39; i > 37; i--) {
                result[i] = sum % 10 + '0';
    ...
    void print(const food* foodList, size_t foodSize) {
        ...
        for (size_t i = 0; i < foodSize; i++) {
            std::cout << [&foodList](int number) -> char* {
                ...
                int shift = 0;
                for (size_t i = 0; i < 20; i++) {
                    if ((int)foodList[number].name[i] < 0) {
                        if (i < strlen(foodList[number].name)) {
                            shift++;
                            result[i] = foodList[number].name[i];
                        } else {
                            if (shift % 2 == 0) {
                                result[i] = ' ';
                            }
                ...
                result[20 + shift] = ' ';
                long cost = foodList[number].cost * 100;
                for (int i = 29; i > 27; i--) {
                    result[i + shift] = cost % 10 + '0';
                    cost /= 10;
                }
                ...
                result[30 + shift] = ' ';
                int quant = foodList[number].quant;
                for (int i = 41; i > 30; i--) {
                    if (quant == 0) {
                        result[i + shift] = ' ';
                    } else {
                ...
                result[42 + shift] = ' ';
                long sum = foodList[number].cost * foodList[number].quant * 100;
                for (int i = 61; i > 59; i--) {
                    result[i + shift] = sum % 10 + '0';
                    sum /= 10;
                }
        ...
        std::cout << "--------------------------------------------------------------" << std::endl
                  << "Всего товаров на сумму" << [&foodList, &foodSize]() -> char* {
            char* result = new char[41];
            double _sum = 0;
            for (size_t i = 0; i < foodSize; i++) {
                _sum += foodList[i].cost * foodList[i].quant;
            }
            long sum = _sum * 100;
            for (int i = 39; i > 37; i--) {
                result[i] = sum % 10 + '0';
